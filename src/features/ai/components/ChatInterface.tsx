      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6" ref={scrollRef}>
        {messages.length === 0 && (
          <div className="h-full flex flex-col items-center justify-center text-muted-foreground space-y-6">
            ...
            <div className="flex flex-wrap justify-center gap-2 max-w-lg">
              <Button variant="outline" size="sm" onClick={() => handleQuickPrompt('Tolong buatkan Review Mingguan dari task saya')} className="text-xs rounded-full">
                <Zap className="w-3 h-3 mr-1" /> Review Mingguan
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleQuickPrompt('Analisis progress goal saya saat ini')} className="text-xs rounded-full">
                <Zap className="w-3 h-3 mr-1" /> Progress Goal
              </Button>
              <Button variant="outline" size="sm" onClick={() => handleQuickPrompt('Apa saja project saya yang sedang aktif?')} className="text-xs rounded-full">
                <Zap className="w-3 h-3 mr-1" /> Project Aktif
              </Button>
            </div>
          </div>
        )}
          ...
          <div key={m.id} className={`flex gap-3 sm:gap-4 ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[85%] sm:max-w-[75%] rounded-2xl px-4 py-3 shadow-sm ${
              ...
              {m.parts?.some((part: any) => part.type === 'reasoning') && (
                <div className="text-xs text-muted-foreground/70 italic border-l-2 pl-2 mb-2 border-muted-foreground/30">
                  {m.parts
              ...
              {m.toolInvocations?.map((toolInvocation: ToolInvocation) => (
                <div key={toolInvocation.toolCallId} className="mt-3 bg-background/50 rounded-lg p-3 text-xs border">
                  <div className="flex items-center gap-2 font-medium mb-1 text-muted-foreground">
            ...
            {m.role === 'user' && (
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-muted flex items-center justify-center shrink-0 border">
        ...
        {isLoading && messages[messages.length - 1]?.role === 'user' && (
          <div className="flex gap-3 sm:gap-4 justify-start">
            ...
            <div className="bg-muted rounded-2xl rounded-tl-sm px-5 py-4 flex items-center gap-1.5 shadow-sm">
              <span className="w-1.5 h-1.5 rounded-full bg-foreground/40 animate-bounce" />
              <span className="w-1.5 h-1.5 rounded-full bg-foreground/40 animate-bounce delay-100" />
              <span className="w-1.5 h-1.5 rounded-full bg-foreground/40 animate-bounce delay-200" />
            </div>
          </div>
        )}
      </div>
